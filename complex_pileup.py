...
def align_to_donor(donor, read):
    ...
    #find an exact consensus match for your starting point to be the seed
    #align to seed
...
def identify_changes(ref, donor, offset):
    ...
    #all indels are less than 5 bp long ***
    ...
    if len(changes) > 0:   #if not empty
        count_snp = 0
        possible_snps = []
        count_indel = 0
        for ele in range(len(changes)):
            if changes[ele][0] == "SNP":
                count_snp += 1
                possible_snps.append(changes[ele])
            if changes[ele][0] == "INS" or changes[ele][0] == "DEL":
                if len(changes[ele][1]) <= 5: #and count_indel < 3:
